...
class ANNEvaluation():
	def __init__(self, output_size):
		try:
			doc = minidom.parse('plugins/SerpentDonkeyKongGameAgentPlugin/files/helpers/config.xml')
			...
			with open('plugins/SerpentDonkeyKongGameAgentPlugin/files/helpers/' + filename + '.json') as f:
	...
	def _create_config_file(self):
		...
		doc.writexml(open('plugins/SerpentDonkeyKongGameAgentPlugin/files/helpers/config.xml', 'w'), indent="    ", addindent="  ", newl='\n')
	...
	def feed(self, inputs):
		print(inputs)
		keys = self._evaluate_network(self.ann["network"], inputs)
		# print(keys)
		return keys
	...
	def fitness(self, mario_positions):		
		score = 4 * mario_positions[1] - mario_positions[0]
